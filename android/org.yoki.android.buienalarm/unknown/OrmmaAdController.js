...
const MRAID_STATE_LOADING = "loading";
...
(function() {
	...
	mraid = {
		...
		state: MRAID_STATE_LOADING,
		lastState: MRAID_STATE_LOADING,
		...
		addEventListener: function(evt, listener) {
			if (evt != ORMMA_EVENT_ERROR && evt != ORMMA_EVENT_HEADING_CHANGE && evt != ORMMA_EVENT_KEYBOARD_CHANGE && evt != ORMMA_EVENT_LOCATION_CHANGE && evt != ORMMA_EVENT_NETWORK_CHANGE && evt != ORMMA_EVENT_ORIENTATION_CHANGE && evt != ORMMA_EVENT_READY && evt != ORMMA_EVENT_RESPONSE && evt != ORMMA_EVENT_SCREEN_CHANGE && evt != ORMMA_EVENT_SIZE_CHANGE && evt != ORMMA_EVENT_STATE_CHANGE && evt != ORMMA_EVENT_TILT_CHANGE && evt != ORMMA_EVENT_ASSET_READY && evt != ORMMA_EVENT_ASSET_REMOVED && evt != ORMMA_EVENT_SHAKE && evt != ORMMA_EVENT_VIEWABLE_CHANGE && evt != ORMMA_EVENT_ASSET_RETIRED) {
				mraid.fireError('addEventListener', 'Unknown event: ' + evt);
			}
		...
		removeEventListener: function(evt, listener) {
			...
			if (typeof listener == 'function' && this.events[evt] && this.events[evt].listeners) {
				...
				mraid.fireError('removeEventListener', 'Listener not currently registered for event \'' + evt + '\'');
			}
		},

		setOrientationProperties: function(orientationProperties) {
            _setOrientationProperties(orientationProperties);
		},

		getOrientationProperties: function() {
            return _getOrientationProperties();
		},
		...
		close: function() {
			if (this.state == ORMMA_STATE_DEFAULT) {
			    console.log("hidding view");
				mraid.hide();
			} else if (this.state == ORMMA_STATE_RESIZED ) {
			    console.log("closing resided view");
			    _close();
			} else {
		...
		show: function() {
			if (this.state == ORMMA_STATE_HIDDEN) {
				...
				mraid.fireError('show', 'Ad is currently visible');
			}
		},
		...
		gotOrientationChange: function(change) {
			...
			fireEvent(ORMMA_EVENT_SCREEN_CHANGE, mraid.getScreenSize());
		},
		...
		setResizeProperties: function(properties) {
			if ( properties.hasOwnProperty("allowOffscreen")) {
			    this.resizeProperties.allowOffscreen = properties.allowOffscreen;
			}
			...
			if ( properties.hasOwnProperty("width")) {
			    this.resizeProperties.width = properties.width;
			}
			if ( properties.hasOwnProperty("height")) {
			    this.resizeProperties.height = properties.height;
			}
			if ( properties.hasOwnProperty("offsetX")) {
				this.resizeProperties.offsetX = properties.offsetX;
			}
			if ( properties.hasOwnProperty("offsetY")) {
				this.resizeProperties.offsetY = properties.offsetY;
			}
		},
		...
		getExpandProperties: function() {
			...
			if (this.expandProperties.width <= 0 || this.expandProperties.width == undefined) {
				this.expandProperties.width = Math.round(ms.width);
			}
			if (this.expandProperties.height <= 0 || this.expandProperties.height == undefined) {
				this.expandProperties.height = Math.round(ms.height);
			}
			return this.expandProperties;
		},

		setExpandProperties: function(properties) {
			var fields = ['width', 'height', 'useCustomClose', 'isModal', 'lockOrientation', 'useBackground', 'backgroundColor', 'backgroundOpacity'];

			for (f in fields) {
				var field = fields[f];
				if (properties[field] !== undefined) {
					if (properties[field] == "width" || properties[field] == "height") {
						this.expandProperties[field] = Math.round(properties[field]);
					} else {
		...
		createCalendarEvent: function(W3Cdata) {
			_createCalendarEvent(W3Cdata);
		},
	...
	function getListenerIndex(event, listener) {
		var len, i;
		if (mraid.events[event] && mraid.events[event].listeners) {
			len = mraid.events[event].listeners.length;
			for (i = len - 1; i >= 0; i--) {
				if (mraid.events[event].listeners[i] === listener) {
					return i;
				}
			}
		}
		...
		if (mraid.events[event] && mraid.events[event].listeners) {
			...
			for (i = len - 1; i >= 0; i--) {
				if (event == ORMMA_EVENT_ERROR) {
					(mraid.events[event].listeners[i])(args.message, args.action);
				} else if (event == ORMMA_EVENT_STATE_CHANGE) {
					(mraid.events[event].listeners[i])(args);
				} else if (event == ORMMA_EVENT_VIEWABLE_CHANGE) {
					(mraid.events[event].listeners[i])(args);
				} else if (event == ORMMA_EVENT_SIZE_CHANGE) {
					(mraid.events[event].listeners[i])(args.width, args.height);
				} else if (event == ORMMA_EVENT_HEADING_CHANGE) {
					(mraid.events[event].listeners[i])(args);
				} else if (event == ORMMA_EVENT_TILT_CHANGE) {
					(mraid.events[event].listeners[i])(args.x, args.y, args.z);
				} else if (event == ORMMA_EVENT_SHAKE) {
					(mraid.events[event].listeners[i])();
					...
					(mraid.events[event].listeners[i])(args.lat, args.lon, args.acc);
				} else if (event == ORMMA_EVENT_NETWORK_CHANGE) {
					(mraid.events[event].listeners[i])(args.online, args.connection);
					...
					(mraid.events[event].listeners[i])(args.uri, args.response);
				} else {
					console.log("Triggering core even: " + event);
					(mraid.events[event].listeners[i])(event, args);
				}
			}
		}
	}
	...
	function _resize(width, height) {
	    // If called with out params it's an MRaid2 call
	    if ( typeof width === "undefined" &&
    		 typeof height === "undefined" ) {
            var resizeProperties = mraid.getResizeProperties();
            if ( typeof resizeProperties === "undefined" ||
                 typeof resizeProperties.width === "undefined" ||
                 typeof resizeProperties.height === "undefined" ) {
                console.log("Illegal resize properties. Did you call setResizeProperties()?");
            }

    		var width = resizeProperties.width;
    		var height = resizeProperties.height;
			var offsetX = 0;
			if( resizeProperties.hasOwnProperty("offsetX"))
				offsetX = resizeProperties.offsetX;
			var offsety = 0;
			if( resizeProperties.hasOwnProperty("offsetY"))
				offsetY = resizeProperties.offsetY;
    		var allowOffscreen = true;
			if( resizeProperties.hasOwnProperty("allowOffscreen"))
				allowOffscreen = resizeProperties.allowOffscreen;
    		var customClosePosition = "top-right";
			if( resizeProperties.hasOwnProperty("customClosePosition"))
				customClosePosition = resizeProperties.customClosePosition;

            ORMMADisplayControllerBridge.resizeMRaid2(
                width, height, offsetX, offsetY, allowOffscreen, customClosePosition);
        } else {
            // Keep backwards compat for MRaid1
    ...
    function _getCurrentPosition() {
        return eval('(' + ORMMADisplayControllerBridge.getCurrentPosition() + ')');
    }

    function _setOrientationProperties(orientationProperties) {
        var allowOrientationChange = true;
        if (orientationProperties.hasOwnProperty("allowOrientationChange")) {
            allowOrientationChange = orientationProperties.allowOrientationChange;
        }
        ORMMADisplayControllerBridge.setOrientationProperties(
            allowOrientationChange,
            orientationProperties.forceOrientation
        );
    }

    function _getOrientationProperties() {
        return eval('(' + ORMMADisplayControllerBridge.getOrientationProperties() + ')');
    }
	...
	function _createCalendarEvent(W3Cdata) {
	    ORMMAUtilityControllerBridge.createCalendarEvent(
	        W3Cdata.description,
	        W3Cdata.location,
	        (W3Cdata.summary || W3Cdata.description),
	        W3Cdata.status,
	        Date.parse(W3Cdata.start),
	        Date.parse(W3Cdata.end)
	    );
	}
...
window.Ormma = window.mraid;

//function setupLegacyHTML5VideoSupport() {		
//	var video = document.getElementsByTagName("video");
//	if ( video.length => 0 ) {
//		video[0].addEventListener("click", function() {
//			mraid.playVideo(video[0].src, 'inline');
//		}, false);		
//	} 
//}

console.log("MRAID Javascript loaded");
